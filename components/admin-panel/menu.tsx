        ...
        <ul className="flex flex-col min-h-[calc(100vh-48px-36px-16px-32px)] lg:min-h-[calc(100vh-32px-40px-32px)] items-start space-y-1 px-2">
          {menuList.map(({ groupLabel, menus }, index) => (
            <li className={cn("w-full", groupLabel ? "pt-6" : "")} key={index}>
              ...
              {menus.map(
                ({ href, label, icon: Icon, active, submenus, isSpecial }, index) =>
                            ...
                            <Button
                              variant={
                                (active === undefined &&
                                  ...
                                  ? "secondary"
                                  : isSpecial
                                  ? "default" // Special styling for K9 Buddy AI
                                  : "ghost"
                              }
                              className={cn(
                                "w-full justify-start h-10 mb-1",
                                isSpecial && "bg-[#f1f5f9] hover:bg-[#e5e7eb] text-[#111827] border border-[#374151] hover:border-[#6b7280] transition-colors duration-150"
                              )}
                              asChild
                            >
                              <Link href={href}>
          ...
          <p className="w-full text-center text-xs text-muted-foreground pt-2">Rit version 2</p>
        </ul>
      </nav>
    </ScrollArea>
  );
